...
function getDhruConfig() {
  const baseUrl = process.env.DHRU_URL || process.env.DHRU_BASE_URL;
  const username = process.env.DHRU_USERNAME || process.env.DHRU_USER;
  const apiKey = process.env.DHRU_API_KEY;
  const timeoutMs = getTimeoutMs();

  if (!baseUrl || !username || !apiKey) {
    const missing = [];
    if (!baseUrl) missing.push('DHRU_URL');
    if (!username) missing.push('DHRU_USERNAME');
