...
async function handleServices(res, config) {
  const actionCandidates = ['services', 'service_list', 'listservices'];
  let result = null;
  let parsed = null;
  let list = [];

  for (const action of actionCandidates) {
    const attempt = await dhruRequest(action);
    if (!result) result = attempt;

    const parsedAttempt = parseJson(attempt.text);
    const listAttempt = extractServiceList(parsedAttempt);

    if (listAttempt.length) {
      result = attempt;
    ...
    if (attempt.response?.ok && parsedAttempt) {
      result = attempt;
      parsed = parsedAttempt;
      list = listAttempt;
      break;
